...
public class All_Ads_Config {
    ...
    int[] images = new int[] {R.drawable.custom_inter_image};

    public All_Ads_Config(Activity activity) {
        this.activity = activity;
        preference = new Ads_Preference(activity);
    }

    public void Custom_Interstitial_Show() {
        if (is_internet(activity)) {
            if (preference.get_custom_ads_on_off().equalsIgnoreCase("ON")) {
                createAndShowDialog();
            }
        }
    }
    ...
    private void createAndShowDialog() {
        Dialog dialog = new Dialog(activity, WindowManager.LayoutParams.MATCH_PARENT);
        ...
        dialog.findViewById(R.id.intall).setBackgroundResource(images[imageId]);
        ...
        dialog.findViewById(R.id.intall).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent();
                intent.setAction(Intent.ACTION_VIEW);
                intent.addCategory(Intent.CATEGORY_BROWSABLE);
                intent.setData(Uri.parse("https://1186.win.qureka.com/intro"));
    ...
    private void Admob_First_Native_Ads(final ViewGroup viewGroup) {
        try {
            adLoader = new AdLoader.Builder(activity, preference.get_admob_native_advance_id())
